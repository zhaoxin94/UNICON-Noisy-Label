...
parser.add_argument('--tau',
                    default=5,
                    type=float,
                    help='filtering coefficient')
parser.add_argument('--data_path',
                    default='./data/Clothing1M_org',
                    type=str,
...
parser.add_argument('--resume',
                    default=False,
                    type=bool,
                    help='Resume from the warmup checkpoint')
...
def train(epoch, net, net2, optimizer, labeled_trainloader,
    ...
    for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x,
        ...
        inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x = inputs_x.cuda(
        ), inputs_x2.cuda(), inputs_x3.cuda(), inputs_x4.cuda(), labels_x.cuda(
        ), w_x.cuda()
        inputs_u, inputs_u2, inputs_u3, inputs_u4 = inputs_u.cuda(
        ), inputs_u2.cuda(), inputs_u3.cuda(), inputs_u4.cuda()
        ...
        sys.stdout.write(
            ...
            % (args.dataset, epoch, args.num_epochs, batch_idx + 1, num_iter,
               loss_x / (batch_idx + 1), loss_ucl / (batch_idx + 1)))
        sys.stdout.flush()
...
def val(net, val_loader, k):
    ...
    if acc > best_acc[k - 1]:
        ...
        save_point = os.path.join(model_save_loc,
                                  '%s_net%d.pth.tar' % (args.id, k))
...
def Calculate_JSD(epoch, model1, model2):
    ...
    for batch_idx, (inputs, targets, path) in enumerate(eval_loader):
        ...
        prob[int(batch_idx * batch_size):int((batch_idx + 1) *
                                             batch_size)] = dist
...
if args.resume:
    net1.load_state_dict(torch.load(os.path.join(model_save_loc,
                                                 model_name_1)))
    net2.load_state_dict(torch.load(os.path.join(model_save_loc,
                                                 model_name_2)))
...
for epoch in range(0, args.num_epochs + 1):
    ...
    if epoch < warm_up:
        ...
        prob2, paths2 = Calculate_JSD(epoch, net1,
                                      net2)  ## Calculate the JSD distances
        threshold = torch.mean(
            prob2)  ## Simply Take the average as the threshold
        if threshold.item() > args.d_u:
            threshold = threshold - (threshold - torch.min(JSD)) / args.tau

        SR = torch.sum(prob2 < threshold).item() / prob2.size()[
            0]  ## Calculate the Ratio of clean samples

        for i in range(nb_repeat):
            print('\n\nTrain Net1')
            labeled_trainloader, unlabeled_trainloader = loader.run(
                SR, 'train', prob=prob2, paths=paths2)  ## Uniform Selection
        ...
        net1.load_state_dict(
            torch.load(
                os.path.join(model_save_loc, '%s_net1.pth.tar' % args.id)))
        ...
        for i in range(nb_repeat):
            print('\nTrain Net2')
            labeled_trainloader, unlabeled_trainloader = loader.run(
                SR, 'train', prob=prob1, paths=paths1)  ## Uniform Selection
